...
export class UsuarioService {
  constructor(
    ...
    @InjectModel(Interacciones.name)
    private interaccionesModel: Model<Interacciones>,
  ) {}
  ...
  async buscarPersonas(usuarioId: string, filtros: any): Promise<Usuario[]> {
    ...
    const interacciones = await this.interaccionesModel
      .find({
    ...
    const usuariosInteractuados = interacciones.map((i) =>
      i.usuarioDestino.toString(),
    );
    console.log(
      'Usuarios interactuados en UsuarioService: ',
      usuariosInteractuados,
    );
  ...
  async agregarSuperLike(
    usuarioId: string,
    usuarioSuperLikeadoId: string,
  ): Promise<void> {
    await this.interaccionesModel.findByIdAndUpdate(usuarioId, {
  ...
  async agregarDislike(
    usuarioId: string,
    usuarioNoGustaId: string,
  ): Promise<void> {
    await this.interaccionesModel.findByIdAndUpdate(usuarioId, {
